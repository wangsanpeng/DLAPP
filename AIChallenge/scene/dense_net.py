...
#        self._images = tf.reshape(images, shape=[-1, self.model_conf.HEIGHT, self.model_conf.WIDTH, 3])
    ...
    def add_layer(self, x, name, activation_fn=tf.nn.relu):
        with tf.variable_scope(name) as scope:
            shortcut = x
            x = conv2d(x, self._growth_rate, kernel_size=3)
    ...
    def bottleneck_layer(self, x, name, activation_fn=tf.nn.relu):
        with tf.variable_scope(name) as scope:
            shortcut = x
            x = conv2d(x, 4*self._growth_rate, kernel_size=1, name=name+"conv2d_1")
            x = batch_norm(x, self._is_training, name=name + 'batch_norm_2')
            x = activation_fn(x)
            x = conv2d(x, self._growth_rate, kernel_size=3, name=name + "conv2d_2")
            x = batch_norm(x, self._is_training, name=name + 'batch_norm_1')
            x = activation_fn(x)
            x = tf.concat([shortcut, x], axis=3)
        return x
    ...
    def add_transition(self, x, name, activation_fn=tf.nn.relu):
        ...
        with tf.variable_scope(name) as scope:
            xx = conv2d(x, input_channel * self._compression_rate, kernel_size=1)
            xx = batch_norm(xx, self._is_training)
            xx = activation_fn(xx)
            xx = avg_pool2d(xx, kernel_size=2)
        return xx

    def dense_net(self, x, activation_fn=tf.nn.relu):
        with tf.variable_scope('densenet_conv1', reuse=self._reuse) as scope:
            x = conv2d(x, output_channel=2*self._growth_rate, kernel_size=7, stride=2)
            x = batch_norm(x, self._is_training)
            x = activation_fn(x)
            x = max_pool2d(x, kernel_size=3)

        with tf.variable_scope('dense_block_1', reuse=self._reuse) as scope:
        ...
        with tf.variable_scope('dense_block_2', reuse=self._reuse) as scope:
            for index in range(self._num_layer_per_block):
                x = self._layer_func(x, name="layer_{}".format(index+1), activation_fn=activation_fn)
        ...
        with tf.variable_scope('dense_block_3', reuse=self._reuse) as scope:
            for index in range(self._num_layer_per_block):
                x = self._layer_func(x, name="layer_{}".format(index+1), activation_fn=activation_fn)
            x = self.add_transition(x, name="transition_3", activation_fn=activation_fn)

        if self.model_conf.DEBUG:
            logger.debug("After dense_block_3 the data shape is {}".format(str(x.get_shape().as_list())))

        with tf.variable_scope('dense_block_4', reuse=self._reuse) as scope:
            for index in range(self._num_layer_per_block):
                x = self.add_layer(x, name="layer_{}".format(index+1), activation_fn=activation_fn)

        with tf.variable_scope('global_pooling', reuse=self._reuse) as scope:
            x = batch_norm(x, self._is_training)
            x = activation_fn(x)
        ...
        with tf.variable_scope('dense_net_end', reuse=self._reuse) as scope:
